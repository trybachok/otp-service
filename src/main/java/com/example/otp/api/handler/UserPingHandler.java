...
public final class UserPingHandler implements HttpHandler {

    private static final Logger logger = LoggerFactory.getLogger(UserPingHandler.class);

    private final AuthMiddleware authMiddleware;
    private final RoleGuard roleGuard;
    private final HttpResponseWriter responseWriter;

    public UserPingHandler(
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            if (!"/api/user/ping".equals(exchange.getRequestURI().getPath())) {
            ...
            logger.warn("Unauthorized user request: {}", exception.getMessage());
            ...
            logger.warn("Forbidden user request: {}", exception.getMessage());
            ...
            logger.error("Unexpected error on user endpoint", exception);
